...
define([
  './util'
    ...
    var Context = function(options) {
        ...
        self.appid = (typeof options.appid==='undefined') ? null : (options.appid || 'dev.appid');
        self.rtAppidDelimeter = options.rtAppidDelimeter || "-";
        // <appid>-<expiry>-<token>-<hash>
        self.rtAppid = (self.appid) ? [
            self.appid,
            // Always fresh for now.
            Math.floor(Date.now()/1000 + 60*60*24*365),
            // Always random for now.
            Util.randomHex(32),
            // Always random for now.
            Util.randomHex(32)
        ].join(self.rtAppidDelimeter) : null;

    	// The primary identity of this instance of the application.
        // Gets reset as soon as the identity is being initialized.
        ...
        self.identityHost = options.identityHost || Util.getHost();
        // The domain to be used when calling the identity provider service.
        self.identityDomain = options.identityDomain || (self._dev && Util.getHost()) || self.domain;
        ...
        // Number of seconds to wait after finder connection is lost and retries finished.
        // Set to `0` to disable reconnects. You must call `<AccountFinder>.reconnect()` yourself
        // when you think connection can be established again.
        self.finderReconnectInterval = (typeof options.finderReconnectInterval !== "undefined") ? options.finderReconnectInterval : 30;   // 30 Seconds
