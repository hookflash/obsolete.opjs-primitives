...
define([
	...
	'cifre/md5',
	'cifre/utils',
], function (Util, Assert, PKI, SHA1, SHA256, RSA, ASN1, HMAC, FORGE_UTIL, AES, MD5, UTILS) {
	...
	function parsePublicPeerFile(publicPeerFile) {
	    ...
	    Assert.isString(A.signature.key.x509Data);

	    info.publicKey = PKI.publicKeyFromAsn1(ASN1.fromDer(base64ToBinary(A.signature.key.x509Data)));
	...
	function base64ToBinary(binary) {
		return FORGE_UTIL.decode64(binary);
	}
	...
	function encryptWithString(secret, iv, data) {
		var key = UTILS.stringToArray(secret);
		var length = data.length;
		var state = new Array(length);
		for (var i = 0; i < length; i++) {
			state[i] = data.charCodeAt(i);
	...
	function decryptWithString(secret, iv, data) {
		var key = UTILS.stringToArray(secret);
        var state = base64ToArray(data);
        AES.cfb.decrypt(state, key, iv);
        return state.map(function(code) {
	...
	function encryptWithPublicKey(publicKey, data) {
		return binaryToBase64(publicKey.encrypt(data));
	}

	function decryptWithPrivateKey(privateKey, data) {
		return privateKey.decrypt(base64ToBinary(data));
   	}

  function stretchPassword(email, password) {
    var passwordStretched = "password-hash:" + password + ":" + email;
    ...
    for ( var i = 0; i < 1000; i++) {
			...
			md.update(passwordStretched + ":" + email);

			passwordStretched = md.digest().toHex();
		}
	...
	return {
		iv: IV,
		encryptWithString: encryptWithString,
		decryptWithString: decryptWithString,
