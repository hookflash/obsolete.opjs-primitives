		...
		var id = Util.randomHex(32);
  		var payload = {
			"request": {
				...
				"$appid": context.rtAppid,
				"$id": id,
				"$handler": handler,
				"$method": method
			}
  		};
  		if (context.rtAppid === null) delete payload.request.$appid;
  		if (extra) {
...
//		if (url.indexOf("hcs-javascript.hookflash.me") !== -1) delete options.contentType;
  ...
  Request.requestPlain = function(payload, method, url) {
    var options = {
		dataType: 'json',
		method: method,
		url: url,
  ...
  Request.postNotifyTo = function(context, target, handler, method, extra) {
  	try {
  		Assert.isObject(context);
  		Assert.isString(context.rtAppid);
  		Assert.isString(context.identityDomain);
		...
		var id = Util.randomHex(32);
  		...
  		if (context.rtAppid === null) delete payload.notify.$appid;
  		if (extra) {
	  		Assert(typeof extra, "object");
	  		for (var key in extra) {
